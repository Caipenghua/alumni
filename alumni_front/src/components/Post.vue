<template>
  <el-container>
    ...
    <el-main>
      ...
      <el-table stripe ref="multipleTable" :data="list.filter(data => !search || data.postTitle.toLowerCase().includes(search.toLowerCase())|| data.postKeyWord.toLowerCase().includes(search.toLowerCase())|| data.postPublisher.toLowerCase().includes(search.toLowerCase()))" style="width: 100%" :default-sort = "{prop: 'id', order: 'descending'}" max-height="550" @selection-change="handleSelectionChange">
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <el-button
              size="mini"
              @click="deletePost(scope.$index, scope.row)">删除贴子</el-button>
...
export default {
  ...
  methods: {
    ...
    updatePost (index, row) {
      this.$router.push({
        path: `/updatePost/${row.alumniNumber}`
      })
    },
    deletePost (index, row) {
      this.$http.delete('http://localhost:80/post/' + row.postId).then(response => {
